...
| 16 | **Queue Monitor** | ✅ **Done 2026-08-12, re-scoped.** The block was "nothing runs in the background"; `app/worker.py` now does. But a worker is **not a queue** — no backlog, so no `queued_at`, no retry/forget/purge — so this records **runs**, not jobs, and the screen is read-only. Its own warning was right: the spec'd version would have been an empty page |
