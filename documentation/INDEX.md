...
| `planning/CORE_PACKAGE_SPLIT_RND.md` | **Can the backend split into `core/` + `<project>/`?** Feasibility R&D: what is already inverted (measured), the six blockers, why the `users → partners` foreign key is the only real decision, and the string-level audit of what an import scan misses | ⏳ **R&D — 2026-08-21, nothing implemented** |
