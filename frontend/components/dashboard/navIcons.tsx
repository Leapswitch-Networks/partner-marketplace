...
export const NAV_ICONS: Record<string, ReactNode> = {
  ...
  /**
   * Partners — a storefront/office building, for the directory's supply side.
   * `PARTNER_DIRECTORY_PLAN.md` § 15: partners are the organisations being
   * onboarded, verified and listed, not people, so `users`' figure glyph would
  ...
  /**
   * Partner Tiers — two overlapping squares, read as "layered" entitlements
   * rather than a flat list. Distinct from `roles`' badge-with-ribbon glyph:
   * a tier grants listing capacity, not permissions.
   */
  partnerTiers: (
    <svg className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.8}>
      <path
