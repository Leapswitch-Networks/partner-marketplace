...
const FIELD_CLASS =
  "block w-full rounded-xl border-2 border-gray-300 bg-white px-3.5 py-2.5 pr-11 text-sm text-gray-900 placeholder-gray-400 shadow-sm outline-none transition focus:border-[#F97316] focus:ring-2 focus:ring-[#F97316]/20 dark:border-gray-600 dark:bg-gray-800 dark:text-gray-100 dark:placeholder-gray-500";
      ...
      <div className="overflow-hidden rounded-2xl bg-white ring-1 ring-gray-100 dark:bg-gray-900 dark:ring-gray-800">
        ...
        <form onSubmit={handleSubmit}>
          <div className="space-y-5 px-6 py-5">
            {grace && (
              <div className="flex items-start gap-3 rounded-lg border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm text-emerald-900 dark:border-emerald-900/40 dark:bg-emerald-950/40 dark:text-emerald-200">
            ...
            {notice && !error && (
              <p className="rounded-lg bg-emerald-50 px-3.5 py-2.5 text-sm text-emerald-700 dark:bg-emerald-950/30 dark:text-emerald-400">
                {notice}
              </p>
            )}
          ...
          <div className="px-6 py-5">
            {!otpOpen ? (
              <button type="button" onClick={() => setOtpOpen(true)}
                className="text-sm font-medium text-[#F97316] transition hover:text-orange-600 hover:underline">
              ...
              <div className="space-y-4">
                ...
                <div className="flex flex-wrap items-end gap-3">
                  <button type="button" onClick={handleSendOtp} disabled={otpBusy}
                    className="rounded-xl border-2 border-gray-300 px-4 py-2 text-sm font-semibold text-gray-700 transition hover:bg-gray-50 disabled:cursor-not-allowed disabled:opacity-50 dark:border-gray-600 dark:text-gray-200 dark:hover:bg-gray-800">
                    ...
                    <>
                      <div>
                        ...
                        <input id="otp" value={otp}
                          ...
                          placeholder="000000"
                          className="w-32 rounded-xl border-2 border-gray-300 bg-white px-3.5 py-2 text-center font-mono text-sm tracking-widest text-gray-900 outline-none transition focus:border-[#F97316] focus:ring-2 focus:ring-[#F97316]/20 dark:border-gray-600 dark:bg-gray-800 dark:text-gray-100" />
                      </div>
                      <button type="button" onClick={handleVerifyOtp}
                        disabled={otpBusy || otp.length !== 6}
                        className="rounded-xl bg-gray-900 px-4 py-2 text-sm font-semibold text-white transition hover:bg-gray-800 disabled:cursor-not-allowed disabled:opacity-50 dark:bg-gray-100 dark:text-gray-900 dark:hover:bg-white">
                        Verify
                      </button>
                    </>
                  )}
