 ...
 * No props.
 *
 * `onNavigate` and `activeSection` were both here to serve the profile modal —
 * one to open it, the other to highlight its menu entry. Profile is a route now, so
 * the component derives "am I in settings" from the pathname and callers no longer
 * have to thread state through. That also fixes the highlight, which broke for any
 * shell that did not happen to pass `activeSection="profile"`.
 */
export default function TopNav() {
  ...
  /**
   * Routes to the settings area rather than calling `onNavigate("profile")`.
   *
   * Profile used to open as a modal, which meant the dropdown only worked where a
   * handler was wired up. Routing directly makes it work from anywhere the top bar
   * renders, and matches LeapDesk, whose user dropdown links to `/settings/profile`.
   */
