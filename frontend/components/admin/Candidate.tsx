...
function Modal({ onClose, children }: { onClose: () => void; children: React.ReactNode }) {
  ...
  return createPortal(
    <div ref={overlayRef} className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4" onMouseDown={(e) => { if (e.target === overlayRef.current) onClose(); }}>
      <div className="w-full max-w-md rounded-none bg-white shadow-xl ring-1 ring-surface-border dark:bg-night-card dark:ring-night-border">
        {children}
      </div>
    </div>,
    document.body
...
function Field({
  ...
  const cls = "block w-full rounded-[5px] border border-surface-border bg-white px-3.5 py-2.5 text-sm text-gray-900 placeholder-gray-400 shadow-sm outline-none transition focus:border-brand focus:ring-2 focus:ring-brand/20 dark:border-night-border dark:bg-night-card dark:text-gray-100 dark:placeholder-gray-500";
  return (
    <div>
      <label htmlFor={id} className="block text-xs font-semibold text-gray-700 mb-1.5 dark:text-gray-300">
        {label}{required && <span className="text-tone-danger ml-0.5">*</span>}
      </label>
      {textarea ? (
...
function AddCandidateModal({ onClose, onSuccess }: { onClose: () => void; onSuccess: (c: Candidate) => void }) {
  ...
  return (
    <Modal onClose={onClose}>
      <div className="flex items-center justify-between border-b border-surface-border px-5 py-4 dark:border-night-border">
        <div>
        ...
        <button type="button" onClick={onClose} className="rounded-[5px] p-1.5 text-gray-400 hover:bg-gray-100 hover:text-gray-600 dark:text-gray-500 dark:hover:bg-gray-800 dark:hover:text-gray-300">
      ...
      <form onSubmit={handleSubmit} className="px-5 py-4 space-y-3">
        ...
        {error && <p className="rounded-[5px] bg-tone-danger/10 px-3 py-2 text-xs font-medium text-tone-danger dark:bg-tone-danger/15 dark:text-tone-danger">{error}</p>}
        <div className="flex gap-2 pt-1">
          <button type="button" onClick={onClose} className="flex-1 rounded-[5px] border border-surface-border px-4 py-2.5 text-sm font-semibold text-gray-600 hover:bg-gray-50 dark:border-night-border dark:text-gray-400 dark:hover:bg-gray-800">Cancel</button>
          <button type="submit" disabled={saving} className="flex-1 rounded-[5px] bg-brand px-4 py-2.5 text-sm font-semibold text-white hover:bg-brand/100 disabled:opacity-60">{saving ? "Adding…" : "Add Candidate"}</button>
        </div>
      </form>
    </Modal>
  );
  ...
  return (
    <Modal onClose={onClose}>
      ...
      <form onSubmit={handleSubmit} className="px-5 py-4 space-y-3">
        ...
        <div className="flex gap-2 pt-1">
          <button type="button" onClick={onClose} className="flex-1 rounded-[5px] border border-surface-border px-4 py-2.5 text-sm font-semibold text-gray-600 hover:bg-gray-50 dark:border-night-border dark:text-gray-400 dark:hover:bg-gray-800">Cancel</button>
          <button type="submit" disabled={saving} className="flex-1 rounded-[5px] bg-brand px-4 py-2.5 text-sm font-semibold text-white hover:bg-brand/100 disabled:opacity-60">{saving ? "Saving…" : "Save Changes"}</button>
        </div>
      </form>
    </Modal>
  );
  ...
  return (
    <Modal onClose={onClose}>
      <div className="flex items-center justify-between border-b border-surface-border px-5 py-4 dark:border-night-border">
        ...
        <button type="button" onClick={onClose} className="rounded-[5px] p-1.5 text-gray-400 hover:bg-gray-100 hover:text-gray-600 dark:text-gray-500 dark:hover:bg-gray-800">
          <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M6 18L18 6M6 6l12 12" /></svg>
        </button>
      </div>
      <div className="px-5 py-4 space-y-4">
        ...
        <div className="flex gap-2">
          <button type="button" onClick={onClose} className="flex-1 rounded-[5px] border border-surface-border px-4 py-2.5 text-sm font-semibold text-gray-600 hover:bg-gray-50 dark:border-night-border dark:text-gray-400 dark:hover:bg-gray-800">Cancel</button>
          <button type="button" onClick={handleDelete} disabled={deleting} className="flex-1 rounded-[5px] bg-tone-danger/100 px-4 py-2.5 text-sm font-semibold text-white hover:bg-tone-danger disabled:opacity-60">{deleting ? "Deleting…" : "Delete"}</button>
        </div>
      </div>
    </Modal>
  );
...
const CandidateTableRow = memo(function CandidateTableRow({
  ...
  return (
    <tr className="hover:bg-brand/10/30 transition-colors dark:hover:bg-brand/20">
      ...
      <td className="px-4 py-3">
        <div className="flex items-center gap-2 justify-end">
          <button type="button" onClick={() => onEdit(c)} className="rounded-[5px] p-1.5 text-gray-400 hover:bg-brand/10 hover:text-brand transition-colors dark:text-gray-500 dark:hover:bg-brand/20 dark:hover:text-brand-on-dark" title="Edit">
            <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" /></svg>
          </button>
          <button type="button" onClick={() => onDelete(c)} className="rounded-[5px] p-1.5 text-gray-400 hover:bg-tone-danger/10 hover:text-tone-danger transition-colors dark:text-gray-500 dark:hover:bg-tone-danger/15 dark:hover:text-tone-danger" title="Delete">
...
const CandidateMobileCard = memo(function CandidateMobileCard({
  ...
  return (
    <div className="rounded-[5px] border border-surface-border bg-white p-4 shadow-sm dark:border-night-border dark:bg-night-card/50">
      <div className="flex items-start justify-between gap-2">
        ...
        <div className="flex gap-1 shrink-0">
          <button type="button" onClick={() => onEdit(c)} className="rounded-[5px] p-1.5 text-gray-400 hover:bg-brand/10 hover:text-brand dark:text-gray-500 dark:hover:bg-brand/20 dark:hover:text-brand-on-dark">
            <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" /></svg>
          </button>
          <button type="button" onClick={() => onDelete(c)} className="rounded-[5px] p-1.5 text-gray-400 hover:bg-tone-danger/10 hover:text-tone-danger dark:text-gray-500 dark:hover:bg-tone-danger/15 dark:hover:text-tone-danger">
            <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" /></svg>
          </button>
        </div>
      </div>
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        ...
        <button type="button" onClick={() => setModal("add")} className="flex items-center gap-2 rounded-[5px] bg-brand px-4 py-2.5 text-sm font-semibold text-white hover:bg-brand/100 transition-colors">
      ...
      {loading && (
        <div className="flex justify-center py-16">
          <svg className="h-6 w-6 animate-spin text-brand dark:text-brand-on-dark" fill="none" viewBox="0 0 24 24">
      ...
      {!loading && error && <div className="rounded-[5px] bg-tone-danger/10 px-4 py-3 text-sm text-tone-danger dark:bg-tone-danger/15 dark:text-tone-danger">{error}</div>}
      ...
      {!loading && !error && candidates.length === 0 && (
        <div className="flex flex-col items-center justify-center py-20 text-center">
          <span className="mb-4 flex h-14 w-14 items-center justify-center rounded-none bg-brand/10 dark:bg-brand/20">
            <svg className="h-7 w-7 text-brand dark:text-brand-on-dark" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0z" /></svg>
          </span>
      ...
      {!loading && !error && candidates.length > 0 && (
        <>
          {/* Desktop table */}
          <div className="hidden md:block overflow-x-auto rounded-[5px] ring-1 ring-surface-border dark:ring-night-border">
      ...
      {toast && (
        <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-50 rounded-[5px] bg-gray-900 px-5 py-3 text-sm font-medium text-white shadow-lg dark:bg-gray-700">
          {toast}
        </div>
      )}
    </div>
